...
conf = TrainerConfig()
experiment.create(name="gpt_math")
experiment.configs(conf, {
    'model.vocab_size': train_dataset.vocab_size,
    'model.block_size': train_dataset.block_size,
    'model.n_layer': 8,
    'model.n_head': 8,
    'model.n_embd': 512,
    'max_epochs': 50,
...
trainer = Trainer(train_dataset, None, conf)
experiment.start()
trainer.train()
...
def give_exam(dataset, batch_size=32, max_batches=-1):
    ...
    for b, (x, y) in enumerate(loader):
        ...
        d1d2 = x[:, :ndigit * 2]
        d1d2d3 = sample(conf.model.model, d1d2, ndigit + 1)
        d3 = d1d2d3[:, -(ndigit + 1):]
